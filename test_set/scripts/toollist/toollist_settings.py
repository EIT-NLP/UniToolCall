...
DATA_NOTIME_DIR = WORKSPACE_ROOT / "test_set" / "data" / "data_nonull"
OUTPUT_DIR = WORKSPACE_ROOT / "test_set" / "data" / "data_toollist"
EMBED_CACHE_DIR = WORKSPACE_ROOT / "tool_set" / "tool_embedding_cache"
QUERY_EMBEDDING_CACHE_DIR = WORKSPACE_ROOT / "test_set" / "list_embedding_cache"
...
API_KEY = ""  # optional ts.API_KEY override; default _api_key()
...
def build_faiss_index_from_tool_pool(
    ...
    """
    Build a Faiss index from the tool pool.

    Returns:
        (faiss_index, index_to_tool_key) or (None, {}) on failure.
    """
    ...
    try:
        ...
        print(f"Built Faiss index: {len(embeddings_list)} tool vectors")
...
def build_toollist_setting(
    ...
    """
    Pick up to 15 tools by cosine similarity to GT, then add 5 random (20 total).
...
def process_file_setting(
    ...
    """Process one file; supports resume."""
    ...
    if output_file.exists():
        try:
            ...
            if isinstance(existing_data, list):
                ...
                processed_indices = set(range(len(existing_data)))
                print(f"  Loaded {len(processed_data)} items from existing output")
        except Exception as exc:
    ...
    # Main loop
    for idx, item in enumerate(tqdm(data, desc=f"Process {input_file.name}")):
        # Skip index if done
...
def main():
    ...
    if not gt_cache:
        # Fallback glob
        # e.g. *test_set*.embeddings.json
        potential_files = list(EMBED_CACHE_DIR.glob("*test_set*.embeddings.json"))
    ...
    json_files = list(DATA_NOTIME_DIR.glob("*.json"))
    ...
    print("=" * 70)
    print("Building toollist (writing data_toollist)...")
    print("=" * 70)
    for input_file in json_files:
        output_file = OUTPUT_DIR / input_file.name
        try:
            process_file_setting(
