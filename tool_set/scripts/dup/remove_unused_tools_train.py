...
def extract_tools_from_data_nonull(data_dir: Path) -> Set[Tuple[str, str]]:
    """
    Collect tool signatures from all JSON files under data_nonull (tools field only).
    ...
    for file_path in json_files:
        try:
            ...
            if file_tools_count > 0:
                ...
                print(f"  {relative_path}: {file_tools_count} unique tool(s)")
    ...
    print(f"  Extractions from tools field: {tools_from_tools_field}")
...
def filter_tools(tool_file_data: Dict, used_tools: Set[Tuple[str, str]]) -> Tuple[Dict, Dict]:
    ...
    for key, tool in tool_file_data.items():
        ...
        is_used = signature in used_tools
...
def main():
    """CLI entry point"""
    tool_file1 = Path("/home/yijuan_liang/10.12Tool_Set/tool_set/apis/apis_nonull/train_set_tool_dedup.json")
    tool_file2 = Path("/home/yijuan_liang/10.12Tool_Set/tool_set/apis/apis_cosdup/train_set_tool_dedup_cosdup.json")
    data_nonull_dir = Path("/home/yijuan_liang/10.12Tool_Set/train_set/data/data_nonull")
    
    print("=" * 80)
    print("Remove unused tools (train set)")
    print("=" * 80)
    ...
    if not data_nonull_dir.exists():
        print(f"Error: data_nonull directory not found: {data_nonull_dir}")
        return
    ...
    stats_file = tool_file1.parent / "train_set_tool_filter_statistics.json"
    statistics = {
