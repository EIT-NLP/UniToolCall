...
def filter_tools(input_file: Path, output_file: Path) -> Dict[str, Any]:
    ...
    if removed_tools:
        print()
        print("Sample removed (first 10):")
        for i, tool_info in enumerate(removed_tools[:10], 1):
...
def main() -> None:
    """CLI entry point"""
    input_file = Path("/home/yijuan_liang/10.12Tool_Set/tool_set/apis/apis_nonull/train_set_tool_dedup.json")
